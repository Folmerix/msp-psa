...
export default function EditQuotePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from("quotes").select("client_id, title, valid_until, tax_rate, notes").eq("id", id).single(),
      supabase.from("line_items").select("item_name, description, quantity, unit_price").eq("quote_id", id).order("sort_order"),
      ...
      if (q.data) { setClientId(q.data.client_id ?? ""); setTitle(q.data.title ?? ""); setValidUntil(q.data.valid_until ?? ""); setTaxRate(String(q.data.tax_rate ?? 0)); setNotes(q.data.notes ?? ""); }
  ...
  async function handleAddClient() {
    ...
    if (data) { const nc = data as Client; setClients(prev => [...prev, nc].sort((a, b) => a.name.localeCompare(b.name))); setClientId(nc.id); }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const { error: uErr } = await supabase.from("quotes").update({
      client_id: clientId || null, title: title || null, valid_until: validUntil || null, notes: notes || null,
    ...
    if (valid.length > 0) await supabase.from("line_items").insert(valid.map((item, idx) => ({ quote_id: id, item_name: item.item_name || null, description: item.description || item.item_name, quantity: parseFloat(item.quantity), unit_price: parseFloat(item.unit_price), sort_order: idx })));
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="text-xl font-bold text-gray-900">Edit Quote</h1>
              </div>
            </div>
          ...
          <div className="space-y-6">
                  ...
                  {!showAddClient ? (
                    ...
                    <div className="mt-3 p-4 bg-blue-50 border border-blue-100 rounded-lg space-y-3">
                      ...
                      <div className="flex gap-2">
                        <button type="button" onClick={handleAddClient} disabled={!newClientName.trim() || addingClient} className="bg-blue-600 text-white rounded-lg px-4 py-2 text-xs font-semibold disabled:opacity-50">{addingClient ? "Adding…" : "Add Client"}</button>
              ...
              <div className="space-y-3">
                {items.map((item, i) => (
                  <div key={i} className="grid grid-cols-[1fr_80px_120px_36px] gap-3 items-start">
                    ...
                    <button type="button" onClick={() => removeItem(i)} disabled={items.length === 1} className="mt-1 text-gray-300 hover:text-red-400 transition disabled:opacity-0 text-lg font-light leading-none">×</button>
                  </div>
                ))}
              </div>
              <button type="button" onClick={addItem} className="mt-4 flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 font-medium transition">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} placeholder="Payment terms, special instructions…" className={inputCls + " resize-none"} />
              </div>
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <div className="space-y-3">
          ...
          <div style={{ overflow: "hidden", boxShadow: "0 1px 8px rgba(0,0,0,0.10)" }}>
            <div style={{ width: 850, zoom: 0.47, background: "white", fontFamily: "Arial, sans-serif", fontSize: 12, color: "#333" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "24px 32px 16px" }}>
                <div>{company?.logo_url ? <img src={company.logo_url} alt={companyName} style={{ height: 52, maxWidth: 220, objectFit: "contain" }} /> : <span style={{ fontSize: 22, fontWeight: 900, color: docP, letterSpacing: 2 }}>{companyName.toUpperCase()}</span>}</div>
                <div style={{ textAlign: "right" }}><p style={{ fontSize: 26, fontWeight: 900, color: docP, letterSpacing: 3, margin: 0 }}>ESTIMATE</p><p style={{ fontSize: 10, color: "#aaa", fontStyle: "italic", margin: "2px 0 0" }}>Live preview</p></div>
              </div>
              <div style={{ display: "flex", gap: 24, padding: "0 32px 16px" }}>
                ...
                <div style={{ width: 220 }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", border: `1px solid #c8d5e4`, fontSize: 11 }}><tbody>
                    <tr><td style={{ backgroundColor: docP, color: "white", fontWeight: 700, padding: "3px 8px", width: 80 }}>Quote #</td><td style={{ padding: "3px 8px", borderBottom: "1px solid #e5eaf0", color: "#aaa", fontStyle: "italic" }}>Editing…</td></tr>
                    ...
                    {validUntil && <tr><td style={{ backgroundColor: docS, fontWeight: 600, padding: "3px 8px", borderBottom: "1px solid #c8d5e4" }}>Valid Until</td><td style={{ padding: "3px 8px" }}>{new Date(validUntil + "T12:00:00").toLocaleDateString()}</td></tr>}
                  </tbody></table>
                </div>
              </div>
              <div style={{ padding: "0 32px 16px" }}>
                <div style={{ backgroundColor: docP, color: "white", fontSize: 10, fontWeight: 700, padding: "2px 8px", marginBottom: 4 }}>PREPARED FOR</div>
              ...
              <div style={{ padding: "0 32px 16px" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                  <thead><tr style={{ backgroundColor: docP, color: "white" }}>
