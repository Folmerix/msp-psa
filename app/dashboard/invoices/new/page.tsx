...
export default function NewInvoicePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data: invoice, error: invErr } = await supabase.from("invoices").insert({
      invoice_number: invoiceNumber, client_id: clientId || null, title: title || null,
      due_date: dueDate || null, notes: notes || null,
      subtotal, tax_rate: parseFloat(taxRate || "0"), tax_amount: taxAmount, total,
      created_by: user?.id ?? null,
    }).select().single();
    if (invErr || !invoice) { setError(invErr?.message ?? "Failed to create invoice"); setLoading(false); return; }
    ...
    if (valid.length > 0) {
      await supabase.from("line_items").insert(valid.map((item, idx) => ({
        invoice_id: invoice.id, item_name: item.item_name || null,
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <div className="space-y-3">
                  ...
                  {parseFloat(taxRate) > 0 && (
                    <div className="flex justify-between text-sm text-gray-500"><span>Tax ({taxRate}%)</span><span>${taxAmount.toFixed(2)}</span></div>
                  )}
